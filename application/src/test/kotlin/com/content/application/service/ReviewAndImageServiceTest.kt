...
class ReviewAndImageServiceTest(
    ...
    private val reviewQueryPort: ReviewQueryPort = mockk(),
        ...
        given("reviewGroup 주어져요") {
            ...
            every { reviewGroupQueryPort.getReviewGroupById(any()) } returns
                ReviewGroup(
                    courseGroupId = 1L,
                    reviewGroupId = 1L,
            ...
            `when`("요청한 코스가 카테고리가 완성되지 않아서, 리뷰 작성 불가능한 경우라면") {
                ...
                then("예외가 발생해요(1)!") {
                    every { courseQueryPort.getCoursesByGroupId(any()) } returns
                        listOf(
                            Course(
                                courseId = 1L,
                                step = 1,
