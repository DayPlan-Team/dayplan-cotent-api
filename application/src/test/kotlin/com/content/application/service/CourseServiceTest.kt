...
class CourseServiceTest(
    ...
    private val reviewQueryPort: ReviewQueryPort = mockk(),
        ...
        given("courseId = 0, placeId = 0인 저장 요청이 주어져요") {
            ...
            `when`("코스 저장 요청을 수행하면") {
                every { courseGroupQueryPort.getCourseGroupById(any()) } returns
                    CourseGroup(
                        userId = 1L,
                        groupId = 1L,
                        groupName = "그룹",
        ...
        given("courseId = 1, placeId = 0인 저장 요청이 주어져요") {
            ...
            `when`("코스를 만든 유저와 동일한 userId면") {
                ...
                every { courseQueryPort.getCourseById(any()) } returns
                    Course(
                        courseId = 1L,
                        step = 1,
                        placeId = 1L,
        ...
        given("courseId = 1, placeId = 1인 저장 요청이 주어져요") {
            ...
            `when`("코스를 만든 유저와 동일한 userId고, place 정보가 존재해요") {
                ...
                every { placePort.getPlaceByPlaceId(any()) } returns
                    listOf(
                        Place(
                            placeName = "",
                            placeCategory = PlaceCategory.CAFE,
            ...
            `when`("코스를 만든 유저와 동일한 userId고, place 정보가 존재하지만, 이미 리뷰가 작성되었어요") {
                ...
                every { reviewQueryPort.getReviewsByCourseGroupId(any()) } returns
                    listOf(
                        Review(
                            reviewId = 1L,
                            reviewGroupId = 1L,
